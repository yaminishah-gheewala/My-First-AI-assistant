    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Lifestyle goals you&apos;re tracking.</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <h2 className="font-semibold text-slate-900 mb-3">Active</h2>
        {active.length === 0 ? (
          <p className="text-sm text-slate-500">No active goals — nice work!</p>
        ) : (
      ...
      {completed.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <h2 className="font-semibold text-slate-900 mb-3">Completed</h2>
          <ul className="space-y-2">
            {completed.map((goal) => (
              <GoalRow
                key={goal.id}
...
function GoalRow({
  ...
  return (
    <li className="flex items-start gap-3 rounded-lg border border-slate-100 px-3 py-2.5">
      <button
        onClick={onToggle}
        disabled={busy}
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${goal.completed ? "text-slate-400 line-through" : "text-slate-800"}`}>
          {goal.title}
        </p>
        {nutrient && (
